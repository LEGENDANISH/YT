/*
  Warnings:

  - The values [FAILED] on the enum `VideoStatus` will be removed. If these variants are still used in the database, this will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "VideoStatus_new" AS ENUM ('UPLOADING', 'UPLOAD_FAILED', 'PROCESSING', 'PROCESSING_FAILED', 'READY', 'CANCELLED', 'DELETED');
ALTER TABLE "Video" ALTER COLUMN "status" DROP DEFAULT;
ALTER TABLE "Video" ALTER COLUMN "status" TYPE "VideoStatus_new" USING ("status"::text::"VideoStatus_new");
ALTER TYPE "VideoStatus" RENAME TO "VideoStatus_old";
ALTER TYPE "VideoStatus_new" RENAME TO "VideoStatus";
DROP TYPE "VideoStatus_old";
ALTER TABLE "Video" ALTER COLUMN "status" SET DEFAULT 'UPLOADING';
COMMIT;

-- AlterTable
ALTER TABLE "UploadChunk" ADD COLUMN     "verifiedAt" TIMESTAMP(3);

-- AlterTable
ALTER TABLE "UploadSession" ADD COLUMN     "lastChunkIndex" INTEGER,
ADD COLUMN     "uploadedBytes" BIGINT NOT NULL DEFAULT 0;

-- AlterTable
ALTER TABLE "VideoQuality" ADD COLUMN     "error" TEXT;
